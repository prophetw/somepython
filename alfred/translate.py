...
def translate(queryStrings,fromLang,toLang):
    ...
    sign = appid+q.decode('utf8')+str(salt)+secretKey
    encodeSign=sign.encode('utf-8')
    m2 = hashlib.md5()
    m2.update(encodeSign)
    sign = m2.hexdigest()
    arguments='?appid='+appid+'&q='+urllib.quote(q)+'&from='+fromLang+'&to='+toLang+'&salt='+str(salt)+'&sign='+sign
